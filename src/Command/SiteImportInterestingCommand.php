...
class SiteImportInterestingCommand extends Command
{
    protected static $defaultName = 'site:import-interesting';
...
//        }
    ...
    protected function loadData()
    {
        ...
        $sql = 'SELECT
                    ...
                    `publication_date`,
                    `publication_time`
                FROM `u6291_ot`.`cs_tree_information_unit`
                ORDER BY `id`';
    ...
    private function clearAll(): void
    {
        ...
        $sql = 'TRUNCATE `v3.oytoy.ru`.`interesting`';
        $mysqli->query($sql);
    }
    ...
    private function saveData(Result $data, SymfonyStyle $io)
    {
        ...
        while ($data_item) {
            ...
            if ($data_item->container_id == 0) {
                $data_item->container_id = 'default';
            }
            $sql = 'INSERT INTO `v3.oytoy.ru`.interesting
                    (id, section_id, name, publication_datetime, image1, image2, text1, text2, active, 
                    created_at, last_modified, html_title, meta_description, meta_keywords)
                    VALUES(
                        ' . $conn->q($data_item->id) . ',
                        ' . $conn->e($data_item->container_id) . ',
