...
class SiteImportReviewsCommand extends Command
{
    const SRC_CONNECTION = 'archiveural';

    protected static $defaultName = 'site:import-reviews';
    ...
    private function saveData(Result $data, SymfonyStyle $io)
    {
        ...
        try {
            while ($data_item) {
                $progress->advance();
                $sql = <<<'SQL'
                    INSERT INTO `ritualarchive`.`review` (`id`, `name`, `created_at`, `last_modified`, `comment`, `email`, `ip_address`, `publication_datetime`, `active`)
                    VALUES (%d, %s, %s, %s, %s, %s, %s, %s, %d);
                SQL;
                $sql = sprintf(
                    $sql,
                    $conn->e($data_item->comment_ID),
                    $conn->q($data_item->comment_author),
                    $conn->q($data_item->comment_date),
                    $conn->q($data_item->comment_date),
                    $conn->q($data_item->comment_content),
                    $conn->q($data_item->comment_author_email),
                    $conn->q($data_item->comment_author_IP),
                    $conn->q($data_item->comment_date),
                    $conn->e($data_item->comment_approved),
                );
                $conn->query($sql);
                $data_item = $data->fetch_object();
            }
