...
class SiteImportSectionCommand extends Command
{
    protected static $defaultName = 'site:import-section';
...
//        }
    ...
    protected function loadData()
    {
        ...
        $sql = 'SELECT
                    ...
                    additional, comment, html_title, meta_description, meta_keywords, sort_order, container_id, 
                    parent_id, url, text1, text2, image1_id, image2_id
                FROM `u6291_ot`.`cs_tree_information_units_section`
    ...
    private function clearAll(): void
    {
        ...
        $mysqli->query($sql);
        $sql = 'TRUNCATE `v3.oytoy.ru`.`section`';
        $mysqli->query($sql);
    ...
    private function saveData(Result $data, SymfonyStyle $io)
    {
        ...
        while ($data_item) {
            ...
            if ($data_item->parent_id == 0) {
                $data_item->parent_id = 'default';
            }
            $sql = 'INSERT INTO `v3.oytoy.ru`.section
                    (id, parent_id, name, sort_order, text, active, 
                    created_at, last_modified, html_title, meta_description, meta_keywords)
                    VALUES(
                        ' . $conn->q($data_item->id) . ',
                        ' . $conn->e($data_item->parent_id) . ',
                        ' . $conn->q($data_item->name) . ',
                        ' . $conn->q($data_item->sort_order) . ',
