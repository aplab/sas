...
class ImageUploader
{
    ...
    public $withGalleryBuilder = false;
    ...
    public function receive(?string $local_path = null)
    {
        ...
        switch ($extension) {
            ...
            case 'jpg' :
                ...
                break;
            default :
                $image_string = file_get_contents($tmp_path);
...
//            }
        ...
        if ($this->withGalleryBuilder) {
            $this->galleryBuilderPlugin($history);
        }
...
//        }
    ...
    protected function galleryBuilderPlugin(HistoryUploadImage $history)
    {
        $image = new Image();
        $image->setName($history->getName());
        $image->setImage($history->getPath());
        $image->setSortOrder($_POST['index'] ?? 0);
        $image->setAlt('');
        $image->setSource('');
        $image->setSourceUrl('');
